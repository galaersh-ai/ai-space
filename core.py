#!/usr/bin/env python3
"""core.py - Self-Evolving Agent with auto-publish"""
...
def run_cycle(session):
    ...
    if jm:
        try:
            ...
            # If output exists, save it
            if p.get("output"):
                save_to_outbox(p["output"], p.get("output_type", "thought"), session)
                log(f"Saved to outbox: {p[output][:50]}...", session)
            else:
                # If output is null, publish thought instead
                if p.get("thought"):
